...
const {BSON_VERSIONS, BSONEXT_VERSIONS}= require('../lib/common.js');
...
const BENCHMARK_PATH = path.resolve(`${__dirname}/../lib/`);
...
(async () => {
  ...
  for (const library of BSON_VERSIONS.concat(BSONEXT_VERSIONS)) {
    await new Task({
      documentPath: path.resolve(`${DOCUMENT_ROOT}/binary_singleFieldDocument.json`),
      library,
      iterations: 1,
      warmup: 1,
  ...
  const resultFile = `./etc/resultsCollected.json`;
